...
public class MeetingHistoryService {
  ...
  public void cancelMeetingHistory(Member member, WatchMeeting meeting) {
    // 같이볼까요 신청기록이 존재하지 않을 경우 예외 발생
    WatchMeetingHistory watchMeetingHistory = meetingHistoryRepository
        .findByMemberAndMeeting(member, meeting)
        .orElseThrow(() -> new MeetingHistoryNotValidException("신청기록이 존재하지 않습니다."));

    // History Update
    meetingHistoryRepository.updateCancelDateTime(LocalDateTime.now(), watchMeetingHistory.getCode());
  }
}
